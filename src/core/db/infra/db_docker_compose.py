...
class DbDockerCompose(DbService):

    def __init__(self, db_configs: DbConfigs):
        super().__init__(db_configs)

    def setup(self):
        logger.info(f"MySQL docker container with '{self.db_name}' database started separately via docker compose.")
    ...
    def shutdown(self):
        logger.info(f"MySQL docker container with '{self.db_name}' database not stopped. "
                    f"It will be stopped separately via docker compose.")
