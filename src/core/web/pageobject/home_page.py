...
class HomePage(WebBaseLoggedInPage):
    ...
    def add_product_to_cart(self, product: Product) -> "HomePage":
        ...
        add_btn = product_elem.find_element(By.XPATH, ".//button[text()=' Add To Cart']")
        add_btn.click()

        # Wait for spinner to appear and disappear
        self.wait_until.element_visible(self.SPINNER)
        self.wait_until.element_invisible(self.SPINNER)
        return self
    ...
    def wait_until_page_loaded(self) -> None:
        self.wait_until.element_visible(self.PRODUCTS_LIST)
    ...
    def _find_product(self, expected_product_name: str) -> WebElement:
        ...
        """
        for product in self.element_actions.find_elements(self.PRODUCTS_LIST):
            product_name = product.find_element(By.CSS_SELECTOR, "b").text
