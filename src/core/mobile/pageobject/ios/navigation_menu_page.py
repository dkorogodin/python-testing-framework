from __future__ import annotations

from typing import TYPE_CHECKING

import allure
from appium.webdriver.common.appiumby import AppiumBy

from src.core.mobile.pageobject.mobile_base_page import MobileBasePage

if TYPE_CHECKING:
    from src.core.mobile.pageobject.ios.login_page import LoginPage


class NavigationMenuPage(MobileBasePage):
    """Page object for the Navigation Menu of the ios application."""

    CART_BTN_LOC = (AppiumBy.ACCESSIBILITY_ID, "Cart-tab-item")
    LOGIN_MENU_ITEM_LOC = (AppiumBy.ACCESSIBILITY_ID, "LogOut-menu-item")

    @allure.step("Go to Login Page.")
    def goto_login_page(self) -> "LoginPage":
        """Go to Login Page."""
        self.element_actions.click(self.LOGIN_MENU_ITEM_LOC)
        from src.core.mobile.pageobject.ios.login_page import LoginPage
        return LoginPage(self.driver)

    def wait_until_page_loaded(self) -> None:
        self.wait_until.element_visible(self.LOGIN_MENU_ITEM_LOC)
