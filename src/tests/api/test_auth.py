...
class TestApiAuth:
    @pytest.fixture
    def auth_service(self, api_service_manager) -> AuthService:
        return api_service_manager.get_auth_service()

    def test_auth(self, auth_service: AuthService, configs_manager: ConfigsManager):
        expected_token: Token = ApiAuthTokenFactory.get_token(
            TokenConstantDataBuilder(configs_manager.web_configs.web_username)
        )
        basic_auth = BasicAuthDataBuilder(
            username=configs_manager.web_configs.web_username,
            password=configs_manager.web_configs.web_password
        )
